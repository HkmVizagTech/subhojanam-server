...
(async () => {
  try {
    ...
    externalService.sendToExternalApi = async (donation, payment) => {
      console.log(
        "TEST STUB: sendToExternalApi called with donation id/name:",
    ...
    receiptService.generateReceipt = async (donation, apiResponse) => {
      console.log(
        "TEST STUB: generateReceipt called. donation:",
        donation._id || donation.name,
        "apiResponse keys:",
    ...
    donationModel.findOneAndUpdate = async (q, update, opts) => {
      console.log(
        "TEST STUB: donationModel.findOneAndUpdate called with",
        q,
        update,
      );
    ...
    donationModel.findByIdAndUpdate = async (id, update) => {
      console.log(
        "TEST STUB: donationModel.findByIdAndUpdate called with",
        id,
        update,
      );
    ...
    const webhookSecret =
      process.env.RAZORPAY_WEBHOOK_SECRET || "subhonojanamwebhook";
    ...
    const res = {
      status: (code) => ({
        send: (msg) => console.log("RES status", code, msg),
        json: (o) => console.log("RES json", o),
      }),
    ...
    if (externalCalled)
      console.log("TEST RESULT: external API was called during webhook flow");
